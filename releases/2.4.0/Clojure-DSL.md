...
`spout-spec` takes as arguments the spout implementation (an object that implements [IRichSpout](https://javadoc.io/doc/org.apache.storm/storm-client/2.4.0/org/apache/storm/topology/IRichSpout.html)) and optional keyword arguments. The only option that exists currently is the `:p` option, which specifies the parallelism for the spout. If you omit `:p`, the spout will execute as a single task.

#### bolt-spec

`bolt-spec` takes as arguments the input declaration for the bolt, the bolt implementation (an object that implements [IRichBolt](https://javadoc.io/doc/org.apache.storm/storm-client/2.4.0/org/apache/storm/topology/IRichBolt.html)), and optional keyword arguments.
